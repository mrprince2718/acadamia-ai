...
export const AppSettingsContext = createContext<AppSettingsContextType>({ language: 'EN', setLanguage: () => {} });
...
const MainApp: React.FC = () => {
  ...
  return (
    <AppSettingsContext.Provider value={{ language, setLanguage }}>
      <div className="flex flex-col md:flex-row h-screen bg-white overflow-hidden academia-ai-root">
        ...
        <header className="md:hidden h-14 border-b border-slate-100 flex items-center justify-between px-4 bg-white/80 backdrop-blur-xl z-[60] sticky top-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
              <button onClick={() => setLanguage('EN')} className={`px-2 py-1 rounded-md text-[9px] font-black ${language === 'EN' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>EN</button>
              <button onClick={() => setLanguage('GU')} className={`px-2 py-1 rounded-md text-[9px] font-black ${language === 'GU' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400'}`}>GU</button>
            </div>
        ...
        {/* Desktop Header Extension with Language Toggle */}
        <div className="hidden md:flex fixed top-4 right-8 z-[70] items-center gap-4">
           <div className="flex bg-white/70 backdrop-blur shadow-sm p-1 rounded-2xl border border-slate-200">
              <button onClick={() => setLanguage('EN')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${language === 'EN' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>English</button>
              <button onClick={() => setLanguage('GU')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${language === 'GU' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>ગુજરાતી</button>
           </div>
        </div>
